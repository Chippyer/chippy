...
public abstract class AbstractLogFeignClientProcessor implements FeignClientProcessor {
    ...
    public Object processAfter(RequestElement element, Object response) {
        if (log.isDebugEnabled()) {
            log.debug(String.format(processAfterLogStr, element.getClassName(), element.getMethodName(),
    ...
    public void processException(RequestElement element, Throwable e) {
        if (log.isErrorEnabled()) {
            log.error(
                String.format(processExceptionLogStr, element.getClassName(), element.getMethodName(), e.getMessage()));
            log.error("具体异常信息-" + e);
        }
    }
